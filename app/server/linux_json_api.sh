...
pending_updates() {
  local yumCmd=$(type -P yum)

  result=$($yumCmd check-update 2>/dev/null | $GREP -c '\.')

  $ECHO "{ \"pending_updates\": $result }" | _parseAndPrint
}

ssl_certificates() {
  local opensslCmd=$(type -P openssl)

  $ECHO -n "["

  local first=1
  local cert_paths=$(ls /etc/nginx/ssl/*.crt /etc/nginx/ssl/*.pem /etc/apache2/ssl/*.crt /etc/apache2/ssl/*.pem /etc/pki/tls/certs/*.crt /etc/ssl/certs/*.pem 2>/dev/null)
  for cert in $cert_paths; do
    if [ -f "$cert" ]; then
      local enddate=$($opensslCmd x509 -enddate -noout -in "$cert" 2>/dev/null | $CUT -d= -f2)
      local subject=$($opensslCmd x509 -subject -noout -in "$cert" 2>/dev/null | $CUT -d= -f2-)
      local issuer=$($opensslCmd x509 -issuer -noout -in "$cert" 2>/dev/null | $CUT -d= -f2-)
      if [ -n "$enddate" ]; then
        [ $first -eq 0 ] && $ECHO -n ","
        first=0
        $ECHO -n "{ \"path\": \"$cert\", \"subject\": \"$subject\", \"issuer\": \"$issuer\", \"expiry\": \"$enddate\" }"
      fi
    fi
  done
...
systemd_failed() {
  local systemctlCmd=$(type -P systemctl)

  result=$($systemctlCmd list-units --state=failed --no-legend 2>/dev/null | $AWK '{
    print "{ \"unit\": \"" $1 "\", \"load\": \"" $2 "\", \"active\": \"" $3 "\", \"sub\": \"" $4 "\", \"description\": \""
...
firewall_overview() {
  local iptablesCmd=$(type -P iptables)

  local chains=$($iptablesCmd -L -n 2>/dev/null | $GREP '^Chain' | $WC -l)
  local rules=$($iptablesCmd -L -n 2>/dev/null | $GREP -v '^Chain\|^target\|^$\|^ pkts' | $GREP -c .)

  local default_input=$($iptablesCmd -L INPUT -n 2>/dev/null | $GREP '^Chain' | $AWK '{print $4}' | $TR -d '()')
  local default_forward=$($iptablesCmd -L FORWARD -n 2>/dev/null | $GREP '^Chain' | $AWK '{print $4}' | $TR -d '()')
  local default_output=$($iptablesCmd -L OUTPUT -n 2>/dev/null | $GREP '^Chain' | $AWK '{print $4}' | $TR -d '()')

  [ -z "$default_input" ] && default_input="none"
  [ -z "$default_forward" ] && default_forward="none"
  [ -z "$default_output" ] && default_output="none"

  $ECHO "{ \"chains\": $chains, \"rules\": $rules, \"default_input\": \"$default_input\", \"default_forward\": \"$default_forward\", \"default_output\": \"$default_output\" }" | _parseAndPrint
}
